...
from regional_transformer_qwenimage import RegionalQwenImageTransformer2DModel
...
class RegionalQwenImagePipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        transformer: RegionalQwenImageTransformer2DModel,
    ):
        super().__init__()
    ...
    def __call__(
        ...
        if 'regional_prompts' in attention_kwargs and 'regional_masks' in attention_kwargs:
            for regional_prompt, regional_mask in zip(attention_kwargs['regional_prompts'], attention_kwargs['regional_masks']):
                print(f"regional_prompt: {regional_prompt}")
                regional_prompt_embeds, regional_prompt_embeds_mask = self.encode_prompt(
        ...
        for mask, cond in regional_inputs:
            if mask is not None: # resize regional masks to image size, the flatten is to match the seq len
                mask = torch.nn.functional.interpolate(mask[None, None, :, :], (H//2, W//2), mode='nearest-exact').flatten().unsqueeze(1).repeat(1, cond.size(1))
            else:
                mask = torch.ones((H*W//4, cond.size(1))).to(device=cond.device)
        ...
        print(f"regional_attention_mask: {regional_attention_mask.shape} with {encoder_seq_len} + {hidden_seq_len}")

        num_of_regions = len(masks)
        print(f"num_of_regions: {num_of_regions}")

        prompt_seq_lens = [mask.shape[1] for mask in masks]
        print(f"prompt_seq_lens: {prompt_seq_lens}")
        ...
        for i in range(num_of_regions):
            ...
            regional_attention_mask[i*prompt_seq_lens[i]:(i+1)*prompt_seq_lens[i], i*prompt_seq_lens[i]:(i+1)*prompt_seq_lens[i]] = True

            # txt attends to corresponding regional img
            print(f"masks[i].shape: {masks[i].shape}")
            regional_attention_mask[i*prompt_seq_lens[i]:(i+1)*prompt_seq_lens[i], encoder_seq_len:] = masks[i].transpose(-1, -2)

            # regional img attends to corresponding txt
            regional_attention_mask[encoder_seq_len:, i*prompt_seq_lens[i]:(i+1)*prompt_seq_lens[i]] = masks[i]
        ...
        regional_txt_seq_lens = [sum(prompt_seq_lens)]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i < mask_inject_steps:
                    ...
                    chosen_prompt_embeds_base = prompt_embeds
                    base_ratio = attention_kwargs['base_ratio']
                    chosen_txt_seq_lens = regional_txt_seq_lens
                else:
                    chosen_prompt_embeds = prompt_embeds
                    chosen_prompt_embeds_base = None
                    base_ratio = None
                    chosen_txt_seq_lens = txt_seq_lens
                ...
                with self.transformer.cache_context("cond"):
                    noise_pred = self.transformer(
                        ...
                        txt_seq_lens=chosen_txt_seq_lens,
                        txt_seq_lens_base=txt_seq_lens,
                        attention_kwargs={'regional_attention_mask': regional_attention_mask if base_ratio is not None else None,},
                        return_dict=False,
                    )[0]
