...
class RegionalQwenImageAttnProcessor2_0:
    def __init__(self):  
        self._attention_backend = None
    def QwenDoubleStreamAttnProcessor2_0_call(
            self,
            attn ,
            hidden_states,  # Image stream
            encoder_hidden_states = None,  # Text stream
        ...
        # # Compute joint attention
        # print("shape of joint_query: ", joint_query.shape, txt_query.shape, img_query.shape)
        # print("shape of joint_key: ", joint_key.shape, txt_key.shape, img_key.shape)
        # print("shape of joint_value: ", joint_value.shape, txt_value.shape, img_value.shape)
        # print("shape of attention_mask: ", attention_mask.shape if attention_mask is not None else None)
        # if attention_mask is not None:
        #     #resize to [3884, 3884], now it is shape of attention_mask:  torch.Size([15404, 15404])
        #     attention_mask = torch.zeros(3884, 3884, dtype=torch.bool, device=joint_query.device)
    ...
    def __call__(self,
        ...
        # print("shape of encoder_hidden_states_mask: ", encoder_hidden_states_mask.shape)
        # print("shape of encoder_hidden_states_base: ", encoder_hidden_states_base.shape)
        # print("shape of base_ratio: ", base_ratio)
        # print("shape of attention_mask: ", attention_mask.shape)
        # print("shape of image_rotary_emb: ", image_rotary_emb.shape)
        if base_ratio is not None:
            attn_output_base = self.QwenDoubleStreamAttnProcessor2_0_call(
                attn = attn,
        ...
        if base_ratio is not None and 'regional_attention_mask' in attention_kwargs:
            regional_attention_mask = attention_kwargs['regional_attention_mask'].to(hidden_states_base.device)

        attn_output = self.QwenDoubleStreamAttnProcessor2_0_call(
            attn = attn,
            hidden_states = hidden_states,
            encoder_hidden_states = encoder_hidden_states,
            encoder_hidden_states_mask = encoder_hidden_states_mask,
            attention_mask = regional_attention_mask,
            image_rotary_emb = image_rotary_emb,
        )
        hidden_states, encoder_hidden_states = attn_output

        if base_ratio is not None:
            # return hidden_states_base, encoder_hidden_states_base, encoder_hidden_states_base
...
class QwenImageTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        encoder_hidden_states_base: torch.Tensor,
        base_ratio: Optional[float],
        temb: torch.Tensor,
        image_rotary_emb: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        image_rotary_emb_base: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        ...
        attn_output = self.attn(
            ...
            encoder_hidden_states_base=txt_modulated_base,
        ...
        img_attn_output, txt_attn_output, txt_attn_output_base = attn_output
        ...
        if base_ratio is not None:
            encoder_hidden_states_base = encoder_hidden_states_base + txt_gate1_base * txt_attn_output_base
            ...
            return encoder_hidden_states, hidden_states, None


class RegionalQwenImageTransformer2DModel(
    ...
    def forward(
        ...
        txt_seq_lens_base: Optional[List[int]] = None,
        ...
        if base_ratio is not None:
            ...
            additional_kwargs["regional_attention_mask"] = None
        ...
        if base_ratio is not None:
            image_rotary_emb_base = self.pos_embed(img_shapes, txt_seq_lens_base, device=hidden_states.device)
        else:
            image_rotary_emb_base = None
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                encoder_hidden_states, hidden_states, encoder_hidden_states_base = block(
                    ...
                    image_rotary_emb=image_rotary_emb,
                    image_rotary_emb_base=image_rotary_emb_base,
                    joint_attention_kwargs=additional_kwargs,
                )
